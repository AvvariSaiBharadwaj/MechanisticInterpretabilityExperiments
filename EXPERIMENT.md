...
- Implemented a new experiment entry point at [experiments/ablation/causal_steering.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/causal_steering.py:0:0-0:0).
- Kept [experiments/ablation/intervention.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/intervention.py:0:0-0:0) unchanged as the legacy scalar-swap baseline.
- Implemented configurable steering modes: `mean_swap`, `additive`, `sigma_scaled`, and `sweep`.
- Implemented configurable direction selection by group or explicit direction name.
- Implemented structured JSON result saving for steering runs.
- Syntax-validated [experiments/ablation/causal_steering.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/causal_steering.py:0:0-0:0) with `python3 -m py_compile`.
- Confirmed that the reusable GP training path lives in [experiments/train.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/train.py:0:0-0:0) and already loads GPT-2 Small, builds [MaskedTransformerCircuit](cci:2://file:///Users/s.avvari/IIITH/precog/Beyond-Components/src/models/masked_transformer_circuit.py:42:0-1753:19), creates a run directory under `svd_logs/`, and saves run-specific artifacts.
- Confirmed that [configs/gp_config.yaml](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/configs/gp_config.yaml:0:0-0:0) already provides the base GP settings we should extend rather than replace.

## Current implementation status

- **Documentation**
  - [EXPERIMENT.md](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/EXPERIMENT.md:0:0-0:0) was created before implementation and is now updated with the training-to-steering workflow.

- **Implemented code**
  - New script: [experiments/ablation/causal_steering.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/causal_steering.py:0:0-0:0)
  - Loads a trained circuit checkpoint and run config through CLI arguments.
  - Reuses GP direction definitions from `DIRECTION_RANGES`.
  - Computes per-direction scalar activations on GP prompts.
  - Applies steering in the final residual stream before `ln_final`.
  - Saves structured output as `causal_steering_<mode>.json`.

- **Validated**
  - Static syntax validation passed.

- **Pending**
  - End-to-end execution against a real local GP run directory with checkpoint artifacts.

## Reusable training workflow

The repository already contains the training path we should reuse to produce the checkpoint for steering experiments.

### Entry point

- [experiments/train.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/train.py:0:0-0:0)
- function: [train_circuit(config, logger)](cci:1://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/train.py:1071:0-1519:18)

### What this function already does

- sets the seed
- selects the runtime device
- creates a timestamped run directory under `svd_logs/`
- loads GPT-2 Small using:
  - `HookedTransformer.from_pretrained(config['model']['name'], cache_dir=config['model']['pretrained_cache_dir'])`
- moves the model to the selected device
- constructs [MaskedTransformerCircuit](cci:2://file:///Users/s.avvari/IIITH/precog/Beyond-Components/src/models/masked_transformer_circuit.py:42:0-1753:19)
- loads the GP training and validation datasets via [load_gp_dataset](cci:1://file:///Users/s.avvari/IIITH/precog/Beyond-Components/src/data/data_loader.py:351:0-399:21)
- runs the circuit-discovery training loop
- writes run-specific artifacts into the run directory

### Why this is the correct path to reuse

This path already defines the exact training environment that our steering experiments should consume afterward. Instead of creating a parallel model-loading stack for training, we should:

- train with [experiments/train.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/train.py:0:0-0:0)
- keep the run directory it creates
- use the resulting checkpoint and run summary as inputs to [experiments/ablation/causal_steering.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/causal_steering.py:0:0-0:0)

### Intended end-to-end workflow

1. Train the GP circuit with [experiments/train.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/train.py:0:0-0:0) and [configs/gp_config.yaml](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/configs/gp_config.yaml:0:0-0:0).
2. Keep the generated run directory under `svd_logs/<experiment_name>_<timestamp>/`.
3. Use the resulting checkpoint artifact and run configuration summary as inputs to the causal steering script.
4. Run scalar steering experiments on the trained GP circuit.

## GP configuration base we are extending

The current base config is [configs/gp_config.yaml](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/configs/gp_config.yaml:0:0-0:0).

### Existing model settings

- model name: `gpt2-small`
- pretrained cache directory: `cache_dir`
...
### Existing output and visualization settings
...
## Planned configuration extension for steering

We should extend the GP setup with a steering-specific configuration section rather than replacing the current GP config.

### Proposed new section

A future config extension should include fields for:

- steering mode
- target genders
- direction group or explicit direction names
- additive delta values
- sigma scaling values
- sweep point count
- steering output directory
- checkpoint path override if needed

For the first implementation, these are passed through CLI arguments in [experiments/ablation/causal_steering.py](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/experiments/ablation/causal_steering.py:0:0-0:0), but the long-term cleaner solution is to move them into a steering config file derived from [gp_config.yaml](cci:7://file:///Users/s.avvari/IIITH/precog/Beyond-Components/configs/gp_config.yaml:0:0-0:0).
